...
Harness engineering has matured from a community practice into a **recognized discipline** with formal taxonomy. [Martin Fowler's canonical treatment](https://martinfowler.com/articles/exploring-gen-ai/harness-engineering.html) defines Agent = Model + Harness, distinguishing between **feedforward controls** (guides that shape agent behavior before execution) and **feedback controls** (sensors that observe and correct behavior during execution). An [arxiv paper (2603.05344)](https://arxiv.org/abs/2603.05344) formalizes the scaffolding/harness/context engineering taxonomy. OpenAI revealed their internal product (1M+ lines of code) uses zero human-written code under harness engineering. Teams report 2-5x reliability gains from harness investment.
